...
/**
 * 当一个频道的 [QGuildChannel.type] 的值等于 [ChannelType.CATEGORY] 时，
 * 此频道代表为一个分组。
 *
 ...
 * 并在有需要的时候通过 [category] 查询并获取真正的对象实例。
 *
...
public interface QGChannelCategoryId : Category, BotContainer, GuildInfoContainer {
    ...
    @JvmAsync(asProperty = true)
    @JvmBlocking(asProperty = true, suffix = "")
    public suspend fun category(): QGChannelCategory
}
 ...
 * @author ForteScarlet
 */
public interface QGChannelCategory : Category, BotContainer, GuildInfoContainer, QGChannelCategoryId {
    /**
     * 所属BOT
     */
    override val bot: QGGuildBot
    ...
    /**
     * 根据当前ID查询一个具体的分组对象实例。
     * 效果同 [QGChannelCategoryId.category], 会查询并得到一个新的实例，
     * 当前实例内信息不变。
     *
     * @throws NoSuchElementException 当未查询到结果（例如查询时分组已被删除）
     * @throws QQGuildApiException api查询期间得到的任何异常
