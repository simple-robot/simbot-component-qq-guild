...
private val simbotInClasspath: Boolean by lazy {
    runCatching {
        ...
        runnerClass.methods.any { it.name == "runInNoScopeBlocking" }.also {
            if (it) {
                apiRequestorLogger.debug("simbot-api is contains in current classpath. Blocking request will use simbot runInNoScopeBlocking.")
            } else {
                apiRequestorLogger.debug("simbot-api is contains in current classpath, but 'runInNoScopeBlocking' method not found. Blocking request will use runBlocking with Dispatchers.IO")
            }
        }
    }.getOrElse {
        if (it is ClassNotFoundException) {
            apiRequestorLogger.debug("simbot-api is not contains in current classpath. Blocking request will use runBlocking with Dispatchers.IO")
        } else {
            apiRequestorLogger.debug("simbot-api classpath check failure: {}. Blocking request will use runBlocking with Dispatchers.IO", it.localizedMessage, it)
        }
        false
    }
}
